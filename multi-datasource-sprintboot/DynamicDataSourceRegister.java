...
import csg.ios.app.utils.DynamicDataSourceContextHolder;
...
//        String typeStr = evn.getProperty("spring.datasource.master.type");
        // 获取数据源类型
        Class<? extends DataSource> clazz;//Class<? extends DataSource> clazz = getDataSourceType(typeStr);
        // 绑定默认数据源参数 也就是主数据源
        DataSource consumerDatasource, defaultDatasource = bind(HikariDataSource.class, defauleDataSourceProperties);
        ...
        for (int i = 0; i < configs.size(); i++) {
            ...
            // 绑定参数
            consumerDatasource = bind(HikariDataSource.class, defauleDataSourceProperties);
